...
vendor: glide.yaml
	...
	docker run --rm \
	  -v $(CURDIR):/go/src/github.com/projectcalico/cni-plugin:rw $$EXTRA_DOCKER_BIND \
		...
		--entrypoint /bin/sh -e LOCAL_USER_ID=$(LOCAL_USER_ID) \
		$(CALICO_BUILD) -c ' \
...
$(DEPLOY_CONTAINER_MARKER): Dockerfile build-containerized fetch-cni-bins
	docker build -f Dockerfile.build -t $(DEPLOY_CONTAINER_NAME) .
	touch $@
...
dist/flannel dist/loopback dist/host-local:
	mkdir -p dist
	$(CURL) -L --retry 5 https://github.com/hitomitak/cni/releases/download/$(CNI_VERSION)/cni-ppc64le-$(CNI_VERSION).tgz | tar -xz -C dist ./flannel ./loopback ./host-local
